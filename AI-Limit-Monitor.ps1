<#
    AI Limit Monitor  —  уведомление о восстановлении лимитов Codex / Claude Code
...
# ==================== НАСТРОЙКИ ====================
$TaskName          = 'AI_Limit_Monitor'
$AppId             = 'AI.Limit.Monitor'
$DataDir           = Join-Path $env:LOCALAPPDATA 'AILimitMonitor'
...
function Register-ToastAppId {
    try {
        ...
        New-ItemProperty -Path $key -Name 'DisplayName'        -Value 'AI Limit Monitor' -PropertyType String -Force | Out-Null
...
$dir = Join-Path $env:LOCALAPPDATA 'AILimitMonitor'
...
function Invoke-Setup {
    ...
    Write-Host '   AI LIMIT MONITOR — установка' -ForegroundColor Cyan
    ...
    $toastOk = Show-Notification -Title 'AI Limit Monitor 🔔' -Message 'Проверка связи. Так будет выглядеть сообщение о сбросе лимитов.'
    if ($toastOk) {
...
function Get-StartupLnk {
    Join-Path ([Environment]::GetFolderPath('Startup')) 'AI Limit Monitor.lnk'
}
...
if ($TestToast) {
    Register-ToastAppId
    $ok = Show-Notification -Title 'AI Limit Monitor 🔔' -Message 'Если вы это видите — уведомления работают.'
...
if (-not $Background) {
    Write-Host '=== AI Limit Monitor — локальная проверка ===' -ForegroundColor Cyan
    foreach ($n in $Sources) {
        $s = Get-SourceState $n
